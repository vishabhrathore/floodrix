...
const FORMULAS = [
    {
        ...
        description: "Empirical flood estimation using catchment area and regional constant C. Widely used in Central & Northern India.",
        ...
        resultUnit: "Cumecs",
        inputFields: [
            { key: "M", label: "Catchment Area", unit: "Km²", default: 24.1, hint: "Total drainage area upstream of bridge" },
            { key: "annual_rain", label: "Annual Average Rainfall", unit: "cm", default: 120, hint: "From rainfall records" },
        ],
        lookupTable: {
    ...
    },
    {
        slug: "ryves-formula",
        name: "Ryve's Formula",
        description: "Empirical method based on distance from coast. Applicable to Tamil Nadu, Karnataka, Andhra Pradesh.",
        category: "Flood Discharge",
        ...
        resultUnit: "Cumecs",
        inputFields: [
            { key: "M", label: "Catchment Area", unit: "Km²", default: 24.1, hint: "Total drainage area" },
            { key: "dist_coast", label: "Distance from Coast", unit: "Km", default: 41, hint: "Determines Ryve's C value" },
        ],
        lookupTable: {
    ...
    },
    {
        ...
        description: "Empirical formula for Western Ghats and Maharashtra catchments.",
        category: "Flood Discharge",
        ...
        resultUnit: "Cumecs",
        inputFields: [
            { key: "M", label: "Catchment Area", unit: "Km²", default: 24.1, hint: "For Western Ghats catchments" },
        ],
        lookupTable: null,
    },
    {
        ...
        description: "Empirical formula with adjustable constant C for moderate to high floods.",
        category: "Flood Discharge",
        ...
        inputFields: [
            { key: "M", label: "Catchment Area", unit: "Km²", default: 24.1, hint: "Total drainage area" },
            { key: "C_cf", label: "Creager's Constant", unit: "—", default: 65, hint: "40–130; 65 for moderate floods" },
        ],
        lookupTable: null,
    },
    {
        ...
        description: "For catchments ≤ 25 sq.km. Uses time of concentration and rainfall intensity.",
        category: "Flood Discharge",
        ...
        inputFields: [
            { key: "M", label: "Catchment Area", unit: "Km²", default: 24.1, hint: "Total drainage area" },
            { key: "L", label: "Basin Length", unit: "Km", default: 1.506, hint: "Longest water travel path" },
            { key: "H_high", label: "Highest Elevation", unit: "m", default: 209, hint: "Max elevation in catchment" },
            { key: "H_low", label: "Lowest Elevation", unit: "m", default: 167, hint: "Min elevation at bridge" },
            { key: "F_24hr", label: "24-hr Rainfall (100-yr)", unit: "cm", default: 25.1, hint: "From IMD isopluvial maps" },
            { key: "P", label: "Runoff Coefficient (P)", unit: "—", default: 0.40, hint: "Table 4.1, IRC:SP:13-2004" },
            { key: "f", label: "Areal Reduction (f)", unit: "—", default: 0.90, hint: "Fig 4.2, IRC:SP:13-2004" },
        ],
        lookupTable: null,
    },
    {
        ...
        inputFields: [
            { key: "M", label: "Catchment Area", unit: "Km²", default: 24.1, hint: "Total drainage area" },
            { key: "T_return", label: "Return Period", unit: "years", default: 100, hint: "Design return period" },
        ],
        lookupTable: null,
    },
];
...
async function main() {
    ...
    for (const formula of FORMULAS) {
        ...
        const existing = await prisma.calcWorkflow.findUnique({
            where: { organizationId_slug: { organizationId: org.id, slug: formula.slug } },
            select: { id: true },
        });
        if (existing) {
            console.log(`  ⏭  ${formula.name} — already exists (${existing.id})`);
            createdWorkflows.push({ id: existing.id, name: formula.name, slug: formula.slug });
            continue;
        }
                ...
                const displayNode = await tx.calcNode.create({
                    data: {
                        ...
                        config: {
                            mode: "single",
                            compare_variables: [
                                { key: formula.resultVariable, method: formula.name, label: formula.name },
                            ],
                            selection_rule: "max",
                            result_variable: "Qd",
                            result_unit: "Cumecs",
    ...
    if (!workspace) {
        workspace = await prisma.workspace.create({
            data: {
                ...
                description: "IRC-compliant peak flood estimation for bridge site design",
                icon: "🌊",
            },
        });
    ...
    for (const wf of createdWorkflows) {
        ...
        console.log(`  ${"".padEnd(30)} http://localhost:3000/calc-workflows/${wf.id}`);
        console.log();
    }
