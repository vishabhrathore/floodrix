...
async function main() {
  ...
  const actors = await Promise.all(
    users.map((u, i) =>
      db.calcActor.upsert({
        where: {
          userId_organizationId: { userId: u.id, organizationId: orgs[i].id },
        },
        update: {},
        create: {
          id: `actor_${u.id}`,
  ...
  const dickenVersion = await db.calcVersion.upsert({
    where: {
      calcWorkflowId_version: { calcWorkflowId: "wf_dicken", version: 1 },
    },
    update: {},
    create: {
      id: "ver_dicken_1",
  ...
  await db.calcWorkflow.update({
    where: { id: "wf_dicken" },
    data: { currentVersionId: dickenVersion.id },
  });

  const scourVersion = await db.calcVersion.upsert({
    where: {
      calcWorkflowId_version: { calcWorkflowId: "wf_scour", version: 1 },
    },
    update: {},
    create: {
      id: "ver_scour_1",
  ...
  await db.calcWorkflow.update({
    where: { id: "wf_scour" },
    data: { currentVersionId: scourVersion.id },
  });

  const ryveVersion = await db.calcVersion.upsert({
    where: {
      calcWorkflowId_version: { calcWorkflowId: "wf_ryves", version: 1 },
    },
    update: {},
    create: {
      id: "ver_ryves_1",
  ...
  await db.calcWorkflow.update({
    where: { id: "wf_ryves" },
    data: { currentVersionId: ryveVersion.id },
  });

  const affluxVersion = await db.calcVersion.upsert({
    where: {
      calcWorkflowId_version: { calcWorkflowId: "wf_afflux", version: 1 },
    },
    update: {},
    create: {
      id: "ver_afflux_1",
  ...
  await db.calcWorkflow.update({
    where: { id: "wf_afflux" },
    data: { currentVersionId: affluxVersion.id },
  });
  ...
  const ratingData = [
    {
      ...
      actorId: actors[1].id,
      wfId: "wf_dicken",
      vId: dickenVersion.id,
      score: 5,
      review: "Very accurate, matches field data.",
    },
    {
      id: "rate_2",
      actorId: actors[2].id,
      ...
      vId: dickenVersion.id,
      score: 5,
      review: "Great for medium catchments.",
    },
    {
      id: "rate_3",
      actorId: actors[0].id,
      ...
      vId: scourVersion.id,
      score: 4,
      review: "Solid IRC:78 implementation.",
    },
    {
      id: "rate_4",
      actorId: actors[3].id,
    ...
    },
    {
      ...
      vId: ryveVersion.id,
      score: 4,
      review: "Good for south India projects.",
    },
  ];
  ...
  for (const r of ratingData) {
    await db.calcRating.upsert({
      where: {
        calcWorkflowId_actorId: { calcWorkflowId: r.wfId, actorId: r.actorId },
      },
      update: {},
      create: {
        id: r.id,
  ...
  await db.tableRegistryItem.upsert({
    ...
    create: {
      ...
      data: [
        { slope_range: "0–10%", f_flat: 0.3, f_rolling: 0.4, f_hilly: 0.5 },
        { slope_range: "10–30%", f_flat: 0.4, f_rolling: 0.55, f_hilly: 0.65 },
        { slope_range: ">30%", f_flat: 0.5, f_rolling: 0.65, f_hilly: 0.8 },
      ],
  ...
  const sessionDefs: Array<{
  ...
  }> = [
    {
      ...
      Q: 152.06,
      daysBack: 2,
      idempotencyKey: "seed-idem-priya-001",
    },
    {
      id: "sess_2",
      wfId: "wf_dicken",
      ...
      Q: 198.4,
      daysBack: 3,
      idempotencyKey: "seed-idem-arjun-001",
    },
    {
      id: "sess_3",
      wfId: "wf_dicken",
  ...
  for (const s of sessionDefs) {
    ...
    const completed =
      s.status === SessionStatus.COMPLETED
        ...
        : null;
    const vars = s.M
      ? { catchment_area: s.M, dicken_c: 11.4, Q_dicken: s.Q }
      : {};
    ...
    await db.calcSession.upsert({
      ...
      create: {
        ...
        pauseReason:
          s.status === SessionStatus.PAUSED ? "Awaiting user input" : null,
      ...
      for (const [i, node] of [inputNode, formulaNode, displayNode].entries()) {
        await db.calcNodeExecution.upsert({
          ...
          create: {
            ...
            result:
              i === 1
                ? { value: s.Q, expression: `11.4 * ${s.M}^(3/4)` }
                : null,
  ...
  for (const a of auditEvents) {
    await db.auditLog.upsert({
      ...
      create: {
        ...
        expiresAt: new Date(
          a.createdAt.getTime() + 2 * 365 * 24 * 60 * 60 * 1000,
        ),
      },
    });
  }
