...
export default function WorkspaceDetailPage() {
  ...
  if (!workspace)
    return <div className="p-20 text-center">Workspace not found</div>;

  const allCalculators = useMemo(
    () => getAllCalculators(workspace.children),
    [workspace],
  );
  const regions = [
    "All",
    ...Array.from(new Set(allCalculators.map((c) => c.formula.region))),
  ];
    ...
    <div className="px-7 md:px-10 py-7 md:py-10 w-full">
      ...
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-10">
        <div className="space-y-2">
          <h3 className="text-h3 font-semibold text-[#0a0a0a] tracking-tight leading-tight">
            {workspace.name}
          </h3>
...
function NavSection({
  ...
  if (item.type === "calculator") {
    const show =
      (activeRegion === "All" || item.formula.region === activeRegion) &&
      (activeType === "All" ||
        item.name.includes(activeType) ||
        item.categoryLabel?.includes(activeType));
    if (!show) return null;
    return (
  ...
  const filteredInTree = allInTree.filter((calc) => {
    const regionMatch =
      activeRegion === "All" || calc.formula.region === activeRegion;
    const typeMatch =
      activeType === "All" ||
      calc.name.toLowerCase().includes(activeType.toLowerCase()) ||
      calc.categoryLabel?.toLowerCase().includes(activeType.toLowerCase());
    return regionMatch && typeMatch;
  });

  if (
    filteredInTree.length === 0 &&
    (activeRegion !== "All" || activeType !== "All")
  )
    return null;
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <span className="text-[11px] text-[#a1a1a1] bg-[#f5f5f5] px-1.5 py-0.5 rounded font-mono">
          {filteredInTree.length}{" "}
          {filteredInTree.length === 1 ? "method" : "methods"}
        </span>
      </button>
...
function CalculatorDiscoveryCard({
  calc,
  workspaceName,
  index,
}: {
