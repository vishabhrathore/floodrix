...
const MATH_FUNCTIONS = [
    ...
    "isPositive", "isPrime", "isZero", "kron", "larger", "largerEq", "lcm",
    "leftShift", "lgamma", "log", "log10", "log1p", "log2", "lsolve", "lup", "lusolve",
    "mad", "map", "matrix", "max", "mean", "median", "min", "mod", "mode", "multiply",
...
function evaluateDoc(
    ...
    const varMeta: Record<string, { name?: string; unit?: string; desc?: string }> = {};
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (trim.startsWith("//")) {
            ...
            if (meta.name || meta.unit || meta.desc) {
                pendingMeta = meta;
                const label = meta.name ? meta.name + (meta.unit ? ` [${meta.unit}]` : "") : content.slice(0, 30);
                decorBuf.push({
                    pos: end,
        ...
        try {
            ...
            if (assignM) {
                ...
                const usedVars = Object.keys(scope)
                    .filter(k => k !== varName && new RegExp(`\\b${k}\\b`).test(rhs));
                const isFormula = usedVars.length > 0;

                if (pendingMeta) varMeta[varName] = { ...(varMeta[varName] ?? {}), ...pendingMeta };
                const meta = varMeta[varName] ?? {};
                const val = typeof result === "number"
                    ? result
                ...
                const varDef: ParsedVariable = {
                    ...
                    value: (typeof val === "number" || typeof val === "string" || typeof val === "boolean")
                ...
                if (isFormula) {
                    formulaList.push({
                        ...
                        expression: rhs,
                        fullExpression: trim,
                        result: typeof val === "number" ? parseFloat((val as number).toFixed(6)) : String(val),
            ...
            decorBuf.push({
                pos: end,
                deco: Decoration.widget({ widget: new ResultWidget(display, "result"), side: 1 }),
            });
        } catch (e) {
    ...
    onParsed?.({
        ...
        outputVariable: lastFormula
            ? {
                ...
                displayLabel: varMeta[lastFormula.outputNotation]?.name || lastFormula.outputNotation,
                unit: varMeta[lastFormula.outputNotation]?.unit,
                description: varMeta[lastFormula.outputNotation]?.desc,
                dataType: "NUMBER",
                value: typeof lastFormula.result === "number" ? lastFormula.result : undefined,
            }
            : undefined,
...
const editorTheme = EditorView.theme({
    ...
    ".cm-content": { padding: "12px 0", caretColor: "var(--color-text-primary,#111)" },
    ...
    ".cm-cursor": { borderLeftColor: "var(--color-text-primary,#111)", borderLeftWidth: "2px" },
...
export const MathEditor = forwardRef<MathEditorRef, MathEditorProps>(
    function MathEditor({ defaultValue = DEFAULT_CODE, onChange, onParsed, className = "" }, ref) {
                ...
                autocompletion({
                    override: [
                        (ctx: CompletionContext): CompletionResult | null => {
                            ...
                            const varOptions = [...new Map(userVars.map(v => [v.name, v])).values()]
                            ...
                            const constOptions = MATH_CONSTANTS
                            ...
                            const fnOptions = MATH_FUNCTIONS
            ...
            const view = new EditorView({
                state: EditorState.create({
                    ...
                    extensions: [
                        ...
                        makeAutocomplete(),
                        linter(
                            (v) => evaluateDoc(v, onParsedRef.current),
                            { delay: 200 },
                        ),
                        editorTheme,
        ...
        return (
            <div className={`rounded-xl border border-border/60 bg-background overflow-hidden ${className}`}>
                ...
                <div className="border-t border-border/40 bg-muted/20 px-4 py-1.5">
                    <p className="text-[10px] text-muted-foreground leading-relaxed">
                        ...
                        <code className="rounded bg-muted px-1">// name: X, unit: Y, desc: Z</code> metadata ·{" "}
                        <code className="rounded bg-muted px-1">Q = C * A^0.75</code> formula
                    </p>
                </div>
            </div>
        );
