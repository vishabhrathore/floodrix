                ...
                <Card key={card.label} className="shadow-none border-border">
                    <CardContent className="p-4">
                        ...
                        <div className="flex items-center mt-1">
                            {card.deltaType === "up" && <ArrowUpRightIcon className="size-3 text-green-600 mr-1" />}
                            {card.deltaType === "down" && <ArrowDownRightIcon className="size-3 text-red-600 mr-1" />}
                            <span className={cn(
                                "text-xs font-medium",
                                card.deltaType === "up" ? "text-green-600" :
                                    card.deltaType === "down" ? "text-red-600" : "text-muted-foreground"
                            )}>
                                {card.delta}
                            </span>
                        </div>
...
export const AdminDashboardQuickCreate = () => {
    const items = [
        { title: "New calc workflow", desc: "IRC hydraulics calculation", icon: CalculatorIcon, color: "text-red-600", bgColor: "bg-red-50" },
        { title: "New automation", desc: "n8n-style automation", icon: ZapIcon, color: "text-blue-600", bgColor: "bg-blue-50" },
        { title: "New workspace", desc: "Folder canvas for org", icon: FolderPlusIcon, color: "text-teal-600", bgColor: "bg-teal-50" },
        { title: "Publish formula", desc: "Add to public registry", icon: FileCodeIcon, color: "text-purple-600", bgColor: "bg-purple-50" },
        { title: "Publish table", desc: "Add system coefficient table", icon: TableIcon, color: "text-amber-600", bgColor: "bg-amber-50" },
        { title: "New organization", desc: "Manually onboard a team", icon: Building2Icon, color: "text-red-600", bgColor: "bg-red-50" },
    ];

    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {items.map((item) => (
                <Card key={item.title} className="shadow-none border-border hover:border-muted-foreground/50 cursor-pointer transition-colors">
        ...
        return (
            <Card className="shadow-none border-border h-full">
                ...
                <CardContent className="p-4 pt-0">
                    <div className="text-xs text-muted-foreground py-4">No critical system alerts.</div>
                </CardContent>
            </Card>
        );
    }
...
export const AdminDashboardOrganizations = () => {
    ...
    const mockOrgs = [
        { name: "Bridge House Consultants", members: 12, workflows: 34, status: "Active", plan: "Pro", initial: "BH" },
        { name: "NH Infra Pvt Ltd", members: 5, workflows: 18, status: "Active", plan: "Free", initial: "NH" },
    ];
    ...
    return (
        <Card className="shadow-none border-border">
            <CardHeader className="p-4 pb-2 flex flex-row items-center justify-between">
                ...
                <Button variant="outline" size="sm" className="text-[10px] h-7 px-2">View all</Button>
            </CardHeader>
                ...
                {mockOrgs.map((org) => (
                    <div key={org.name} className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <div className="text-xs text-muted-foreground">{org.members} members · {org.workflows} workflows</div>
                        </div>
                        <div className="flex gap-1">
                            <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200 text-[10px] px-1.5 py-0 h-5">Active</Badge>
                            <Badge variant="outline" className={cn("text-[10px] px-1.5 py-0 h-5", org.plan === "Pro" ? "bg-blue-50 text-blue-700 border-blue-200" : "bg-gray-50 text-gray-700 border-gray-200")}>{org.plan}</Badge>
                        </div>
                    </div>
                ))}
            </CardContent>
...
export const AdminDashboardAuditLog = () => {
    const mockEvents = [
        { time: "2 min ago", actor: "Priya M.", org: "Bridge House", action: "Published", resource: "Flood calc v3", actionColor: "text-blue-600 bg-blue-50 border-blue-200" },
        { time: "14 min ago", actor: "Arjun K.", org: "TechRoads", action: "Run completed", resource: "Scour depth session", actionColor: "text-green-600 bg-green-50 border-green-200" },
    ];
    ...
    return (
        <Card className="shadow-none border-border">
            <CardHeader className="p-4 pb-2 flex flex-row items-center justify-between">
                ...
                <Button variant="outline" size="sm" className="text-[10px] h-7 px-2">Full audit log</Button>
            </CardHeader>
                        ...
                        <tbody>
                            {mockEvents.map((event, i) => (
                                <tr key={i} className="border-b last:border-0 hover:bg-muted/10">
                                    <td className="p-3 text-xs text-muted-foreground whitespace-nowrap">{event.time}</td>
                                    <td className="p-3 text-xs">{event.actor}</td>
                                    <td className="p-3 text-xs">{event.org}</td>
                                    <td className="p-3 text-xs">
                                        <Badge variant="outline" className={cn("text-[10px] px-1.5 py-0 h-5 font-normal", event.actionColor)}>
                                            {event.action}
                                        </Badge>
                                    </td>
