import { CollaboratorPermission, LibraryStatus, Visibility, WorkflowStatus } from "@/generated/prisma";
import { RequestContext } from "@/server/context/context.types";
import { isOrgAdmin, isOrgMember } from "@/server/context/permission";

export function getCollaboratorPermission(ctx: RequestContext): CollaboratorPermission | null {
    if (!ctx.workflow || !ctx.actor) return null;
    const collab = ctx.workflow.collaborators?.find((c) => c.actorId === ctx.actor?.id);
...
export function canEditWorkflow(ctx: RequestContext): boolean {
    ...
    return perm === CollaboratorPermission.EDIT || perm === CollaboratorPermission.ADMIN;
}
...
export function canRunWorkflow(ctx: RequestContext): boolean {
    if (!ctx.workflow) return false;
    if (
        ctx.workflow.libraryStatus === LibraryStatus.LISTED && ctx.workflow.status === WorkflowStatus.PUBLISHED
    ) {
        return true;
    }
