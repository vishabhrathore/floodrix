...
import { z } from "zod";
import {
    AlertCircle, Plus, Trash2, Save, ChevronDown, ChevronRight,
    FunctionSquare, Variable, Info, Check, X, Tag, BookOpen,
    Loader2, GripVertical, Wand2, ShieldCheck, Activity,
} from "lucide-react";
...
import { MathEditor, type EditorParsed } from "@/components/core/MathEditor";
...
const CATEGORIES = [
    "Flood Discharge", "Scour Depth", "Waterway Width", "Rainfall",
    "Velocity", "Afflux", "Sediment", "General",
];

const STANDARDS = [
    "IRC:SP:13-2004", "IRC:78-2014", "IRC:5-2015",
    "IS:11532-1985", "RDSO Guidelines", "Custom",
];
...
function VariableRow({
    prefix, onRemove, register, errors,
}: {
    prefix: string;
    onRemove?: () => void;
        ...
        <div className="group flex items-start gap-2 rounded-md border border-border/40 bg-muted/20 p-3">
            ...
            <div className="flex-1 space-y-2">
                <div className="grid grid-cols-2 gap-2">
                    <div>
                        <Label className="text-[10px] uppercase tracking-wide text-muted-foreground">Notation *</Label>
                        <Input {...register(`${prefix}.notation`)} placeholder="Q" className="mt-1 h-7 font-mono text-xs" />
                    ...
                    <div>
                        ...
                        <Input {...register(`${prefix}.displayLabel`)} placeholder="Peak Discharge" className="mt-1 h-7 text-xs" />
                    </div>
                </div>
                <div className="grid grid-cols-2 gap-2">
                    <div>
                        <Label className="text-[10px] uppercase tracking-wide text-muted-foreground">Unit</Label>
                        <Input {...register(`${prefix}.unit`)} placeholder="m³/s" className="mt-1 h-7 text-xs" />
                    </div>
                    <div>
                        <Label className="text-[10px] uppercase tracking-wide text-muted-foreground">Description</Label>
                        <Input {...register(`${prefix}.description`)} placeholder="Optional" className="mt-1 h-7 text-xs" />
                    </div>
                </div>
            </div>
            {onRemove && (
...
export function FormulaForm({
    ...
    const {
        ...
        defaultValues: {
            visibility: "PRIVATE",
            inputVariables: [{ notation: "", displayLabel: "", unit: "", description: "", dataType: "NUMBER" }],
    ...
    const { fields: inputFields, append, remove, replace } = useFieldArray({
        control, name: "inputVariables",
    });
                ...
                <div className="flex items-center gap-2">
                    {isEditing && onDelete && (
                        <Button
                            ...
                            onClick={() => { if (window.confirm("Delete this formula?")) onDelete(); }}
                        >
                            {isDeleting
                    ...
                    {editorOpen && (
                        <div className="space-y-3 p-4">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                >
                                    {autoFillFlash
                                        ? <><Check className="h-3.5 w-3.5 text-emerald-500" />Applied!</>
                                        : <><Wand2 className="h-3.5 w-3.5" />Auto-fill form from editor</>}
                                </Button>
                ...
                <Section title="Basic Information" icon={Info}>
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="col-span-2">
                            ...
                            <Textarea {...register("description")} placeholder="Describe when and how to apply…" className="mt-1 min-h-[72px] resize-none text-sm" />
                        </div>
                    </div>
                </Section>
                ...
                <Section title="Expression" icon={FunctionSquare}>
                    <div className="space-y-4">
                        <div>
                            ...
                            <Input {...register("expressionNotation")} placeholder="Q = C * A^(3/4)" className="mt-1 h-9 font-mono text-sm" />
                        ...
                        <div>
                            ...
                            <Input {...register("displayExpression")} placeholder="Q = C × A³/⁴" className="mt-1 h-9 text-sm" />
                        ...
                        {expressionNotation && (
                            <div className="rounded-md border border-emerald-200/60 bg-emerald-50/50 px-3 py-2 dark:border-emerald-800/40 dark:bg-emerald-950/20">
                                <p className="mb-1 text-[10px] uppercase tracking-wide text-emerald-700 dark:text-emerald-400">Preview</p>
                                <code className="font-mono text-sm text-emerald-800 dark:text-emerald-300">{expressionNotation}</code>
                            </div>
                        )}
                    </div>
                </Section>
                    ...
                    <div className="space-y-6">
                        <div>
                            <div className="mb-2 flex items-center justify-between">
                                <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Input Variables *</p>
                                <Button
                                    type="button" variant="ghost" size="sm"
                                    onClick={() => append({ notation: "", displayLabel: "", unit: "", description: "", dataType: "NUMBER" })}
                            ...
                            <div className="space-y-2">
                                {inputFields.map((field, index) => (
                                    <VariableRow
                                        ...
                                        register={register}
                                        errors={(errors.inputVariables?.[index] ?? {}) as Record<string, unknown>}
                                    />
                                ))}
                            </div>
                        </div>
                        ...
                        <div>
                            <p className="mb-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">Intermediate Steps</p>
                            <p className="mb-3 text-[11px] text-muted-foreground">Optional steps defined during calculation for clarity.</p>
                            ...
                            <div className="rounded-md border border-dashed border-border/60 p-4 text-center">
                                <p className="text-xs text-muted-foreground">No intermediate steps defined.</p>
                            </div>
                        </div>
                        ...
                        <div>
                            <p className="mb-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">Output Variable *</p>
                            <VariableRow
                                prefix="outputVariable"
                                register={register}
                                errors={(errors.outputVariable ?? {}) as Record<string, unknown>}
                            />
                        </div>
                    </div>
                </Section>
                ...
                <Section title="Source & Reference" icon={BookOpen} defaultOpen={false}>
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <Controller name="sourceStandard" control={control} render={({ field }) => (
                                <Select value={field.value ?? ""} onValueChange={field.onChange}>
                                    <SelectTrigger className="mt-1 h-8 text-sm"><SelectValue placeholder="Select standard" /></SelectTrigger>
                        ...
                        <div>
                            ...
                            <Input {...register("yearIntroduced")} type="number" placeholder="1965" className="mt-1 h-8 text-sm" />
                        </div>
                        <div>
                        ...
                        <div className="col-span-2">
                            ...
                            <Textarea {...register("applicability")} placeholder="Applicable for catchment areas up to 250 km²…" className="mt-1 min-h-[60px] resize-none text-sm" />
                        </div>
                        <div className="col-span-2">
                            <Label className="text-xs">Limitations</Label>
                            <Textarea {...register("limitations")} placeholder="Not valid for coastal areas…" className="mt-1 min-h-[60px] resize-none text-sm" />
                        </div>
                    </div>
                </Section>
                    ...
                    <div className="space-y-4">
                        <div>
                            ...
                            {tags.length > 0 && (
                                <div className="mt-2 flex flex-wrap gap-1">
                                    {tags.map(t => (
                                        <Badge key={t} variant="secondary" className="h-6 cursor-pointer text-xs"
                        ...
                        <div className="flex items-center justify-between rounded-md border border-border/60 p-3">
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground">Allow other organisations to view this formula</p>
                            </div>
            ...
            {isSuperAdminUser && (
                <div className="mt-8 border-t border-border/60 bg-muted/10 p-6">
                    <div className="mb-6 flex items-center gap-2">
                        ...
                        <h3 className="text-sm font-semibold uppercase tracking-wider">Admin &amp; System Settings</h3>
                        <Badge variant="outline" className="ml-2 bg-primary/5 text-[10px] text-primary">SuperAdmin Only</Badge>
                    </div>
                    ...
                    <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                        <div className="space-y-4">
                            <div className="flex items-center justify-between rounded-md border border-border/60 bg-background p-3">
                                <div>
                                    ...
                                    <p className="text-xs text-muted-foreground">Mark as an official system-wide standard</p>
                                </div>
                                <Controller name="isSystem" control={control} render={({ field }) => (
                                    <Switch
                                        checked={field.value}
                            ...
                            <div className="flex items-center justify-between rounded-md border border-border/60 bg-background p-3">
                                <div>
                                    ...
                                    <p className="text-xs text-muted-foreground">Make available for general use in workflows</p>
                                </div>
                        ...
                        <div className="space-y-2 rounded-md border border-border/60 bg-background p-4">
                            ...
                            <div className="grid grid-cols-2 gap-y-2 text-[11px]">
                                ...
                                <code className="truncate font-mono">{(initialValues as any)?.id || "—"}</code>

                                <span className="text-muted-foreground">Created By:</span>
                                <span>{(initialValues as any)?.createdBy || "—"}</span>

                                <span className="text-muted-foreground">Created At:</span>
                                <span>{(initialValues as any)?.createdAt ? new Date((initialValues as any).createdAt).toLocaleString() : "—"}</span>

                                <span className="text-muted-foreground">Updated At:</span>
                                <span>{(initialValues as any)?.updatedAt ? new Date((initialValues as any).updatedAt).toLocaleString() : "—"}</span>

                                <span className="text-muted-foreground">Version:</span>
                                <span>{(initialValues as any)?.currentVersion || "1"}</span>

                                <span className="text-muted-foreground">Deleted At:</span>
                                <span className="text-destructive">{(initialValues as any)?.deletedAt ? new Date((initialValues as any).deletedAt).toLocaleString() : "—"}</span>

                                <span className="text-muted-foreground">Organization ID:</span>
                                <span className="truncate font-mono">{(initialValues as any)?.organizationId || "—"}</span>
                            </div>
                        </div>
                    </div>
                </div>
