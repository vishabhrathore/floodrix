...
import { Save, ArrowLeft, BookOpen, Trash2, Tag, ShieldCheck, Activity, Info, Zap, Globe, Lock, X } from "lucide-react";
...
import { useTable, useCreateTable, useUpdateTable, useDeleteTable } from "../hooks/use-tables";
                                ...
                                <AlertDialogContent>
                                    <AlertDialogHeader>
                                        ...
                                        <AlertDialogDescription>
                                            ...
                                            table <span className="font-semibold">{existing?.name}</span> and remove its data from our servers.
                                        </AlertDialogDescription>
                                    </AlertDialogHeader>
                                    <AlertDialogFooter>
                ...
                <div className="flex-1 overflow-y-auto p-6">
                    <div className="max-w-5xl mx-auto space-y-8">
                                    ...
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                ...
                                                <FormControl>
                                                    <Input placeholder="Empirical, Structural, etc." {...field} value={field.value ?? ""} />
                                                </FormControl>
                                                <FormMessage />
                                            </FormItem>
                                        )}
                        ...
                        <TableDataEditor
                            ...
                            onDataChange={(d: any[]) => form.setValue("data", d, { shouldDirty: true, shouldValidate: true })}
                            onColumnsChange={(c: any[]) => form.setValue("columns", c, { shouldDirty: true, shouldValidate: true })}
                        />
                                ...
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem>
                                            ...
                                            <FormControl>
                                                <Input placeholder="p. 47" {...field} value={field.value ?? ""} />
                                            </FormControl>
                                            <FormMessage />
                                        </FormItem>
                                    )}
                        ...
                        <div className="space-y-6">
                            <div className="flex items-center gap-2">
                                ...
                                <h2 className="text-lg font-semibold">Logic & Fallback Behavior</h2>
                            </div>
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                ...
                                                <Select onValueChange={field.onChange} defaultValue={field.value} value={field.value}>
                                                    ...
                                                    <SelectContent>
                                                        <SelectItem value="error">Throw Error (Strict)</SelectItem>
                                                        <SelectItem value="null">Return Null / Empty</SelectItem>
                                                        <SelectItem value="value">Use Fallback Value</SelectItem>
                                                    </SelectContent>
                                                </Select>
                                                <FormDescription>Determines engine behavior when input is out of range or missing.</FormDescription>
                                                <FormMessage />
                                            </FormItem>
                                        )}
                                    />
                                        ...
                                        <FormField
                                            ...
                                            render={({ field }) => (
                                                <FormItem>
                                                    ...
                                                    <FormControl>
                                                        <Input placeholder="0, N/A, or 1.0" {...field} value={field.value ?? ""} />
                                                    </FormControl>
                                                    <FormMessage />
                                                </FormItem>
                                            )}
                                ...
                                <div>
                                    ...
                                    <div className="flex flex-wrap gap-2 mt-3">
                                        {(form.watch("tags") || []).map((t: string) => (
                                            <Badge key={t} variant="secondary" className="gap-1 px-2 py-1">
                                                {t}
                                                <X
                                                    className="h-3 w-3 cursor-pointer hover:text-destructive"
                                                    onClick={() => form.setValue("tags", (form.getValues("tags") || []).filter((x: string) => x !== t))}
                                                />
                                            </Badge>
                                        ))}
                                    </div>
                            ...
                            <>
                                ...
                                <div className="rounded-xl border border-primary/20 bg-primary/5 p-6 space-y-6">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <h2 className="text-lg font-bold uppercase tracking-tight text-primary">System Administration</h2>
                                        <Badge variant="outline" className="bg-primary/10 border-primary/20 text-primary text-[10px]">SuperAdmin Only</Badge>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div className="space-y-4">
                                            <div className="flex items-center justify-between rounded-lg border bg-background p-4 shadow-sm">
                                                <div className="space-y-0.5">
                                                    <p className="text-sm font-semibold">Official System Table</p>
                                                    <p className="text-xs text-muted-foreground">Mark as a verified, immutable system registry item.</p>
                                                </div>
                                                <FormField
                                                    control={form.control}
                                                    name="isSystem"
                                            ...
                                            <div className="flex items-center justify-between rounded-lg border bg-background p-4 shadow-sm">
                                                <div className="space-y-0.5">
                                                    <p className="text-sm font-semibold">Published Status</p>
                                                    <p className="text-xs text-muted-foreground">Make this table visible in the global library.</p>
                                                </div>
                                                <FormField
                                                    control={form.control}
                                                    name="isPublished"
                                        ...
                                        <div className="rounded-lg border bg-background p-4 shadow-sm space-y-3">
                                            <div className="flex items-center gap-2 mb-1">
                                                ...
                                                <p className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground">Database Metadata</p>
                                            </div>
                                            <div className="grid grid-cols-2 gap-y-1.5 text-[11px] font-mono">
                                                <span className="text-muted-foreground">ID:</span>
                                                <span className="truncate">{existing?.id || "NEW"}</span>
                                                <span className="text-muted-foreground">OrgID:</span>
                                                <span className="truncate">{existing?.organizationId || "-"}</span>
                                                <span className="text-muted-foreground">Creator:</span>
                                                <span className="truncate">{existing?.createdBy || "-"}</span>
                                                <span className="text-muted-foreground">Created:</span>
                                                <span>{existing?.createdAt ? new Date(existing.createdAt).toLocaleDateString() : "-"}</span>
                                                <span className="text-muted-foreground">Updated:</span>
                                                <span>{existing?.updatedAt ? new Date(existing.updatedAt).toLocaleDateString() : "-"}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
