...
export function ExactTableEditor({
  ...
  const updateRow = (idx: number, patch: any) =>
    !readOnly &&
    onChange(data.map((r, i) => (i === idx ? { ...r, ...patch } : r)));
  const removeRow = (idx: number) =>
    !readOnly && onChange(data.filter((_, i) => i !== idx));
    ...
    <div className="space-y-2">
      ...
      <div className="space-y-1.5">
        {data.map((row, idx) => (
          <div
            key={idx}
            className="grid grid-cols-[80px_80px_1fr_80px_40px] items-center gap-2"
          >
            <Input
              type="number"
              step="any"
        ...
        {data.map((row, idx) => (
          ...
          >
            <Input
              ...
              value={row.x}
              onChange={(e) =>
                updateRow(idx, { x: parseFloat(e.target.value) || 0 })
              }
              className="h-8 text-xs font-mono"
              disabled={readOnly}
            />
            <Input
              ...
              value={row.y}
              onChange={(e) =>
                updateRow(idx, { y: parseFloat(e.target.value) || 0 })
              }
              className="h-8 text-xs font-mono"
              disabled={readOnly}
            />
