...
import { useTable, useDeleteTable } from "../hooks/use-tables";
                    ...
                    <div>
                        <div className="flex items-center gap-2">
                            ...
                            <div className="flex items-center gap-1.5">
                                {table.isSystem && (
                                    <Badge variant="secondary" className="h-5 gap-1 px-1.5 text-[10px] bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300 border-blue-200 dark:border-blue-800">
                                        <ShieldCheck className="h-2.5 w-2.5" />
                                        System
                                    </Badge>
                                )}
                                {table.isPublished ? (
                                    <Badge variant="secondary" className="h-5 gap-1 px-1.5 text-[10px] bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300 border-emerald-200 dark:border-emerald-800">
                                        <Eye className="h-2.5 w-2.5" />
                                        Published
                                    </Badge>
                                ) : (
                                    <Badge variant="secondary" className="h-5 gap-1 px-1.5 text-[10px] bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400 border-slate-200 dark:border-slate-700">
                        ...
                        <AlertDialogContent>
                            <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>
                                    ...
                                    table <span className="font-semibold">{table.name}</span> and remove its data from our servers.
                                </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
            ...
            <div className="flex-1 overflow-y-auto p-8">
                <div className="mx-auto max-w-5xl space-y-10">
                    ...
                    <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
                        ...
                        <div className="space-y-1">
                            <p className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Logic & Behavior</p>
                            <div className="flex flex-col gap-1.5">
                                <div className="flex items-center gap-1.5 text-sm">
                                    <Zap className="h-3.5 w-3.5 text-amber-500" />
                                    <span className="text-xs">
                                        {table.fallbackMode === 'error' ? 'Strict' : table.fallbackMode === 'null' ? 'Null Fallback' : `Fallback: ${table.fallbackValue ?? 'None'}`}
                                    </span>
                                </div>
                                <div className="flex gap-2">
                                    <Badge variant="outline" className={`text-[9px] px-1 h-4 ${table.allowOverride ? 'border-blue-200 text-blue-600' : 'opacity-50'}`}>
                                        {table.allowOverride ? 'Override On' : 'No Override'}
                                    </Badge>
                                    <Badge variant="outline" className={`text-[9px] px-1 h-4 ${table.showInOutput ? 'border-emerald-200 text-emerald-600' : 'opacity-50'}`}>
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="flex flex-wrap gap-1.5">
                                    {(table.tags as string[]).map(tag => (
                                        <Badge key={tag} variant="outline" className="bg-muted/30 text-[10px] py-0 px-2 font-normal">
                                            {tag}
                                        </Badge>
                                    ))}
                                </div>
                    ...
                    {isSuperAdminUser && (
                        <div className="mt-6 rounded-xl border border-primary/20 bg-primary/5 p-6 space-y-4">
                            <div className="flex items-center gap-2">
                                ...
                                <h3 className="text-xs font-bold uppercase tracking-wider text-primary">System Metadata</h3>
                                <Badge variant="outline" className="bg-primary/10 border-primary/20 text-primary text-[9px] h-4">SuperAdmin Only</Badge>
                            </div>
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-6 text-[11px] font-mono">
                                <div className="space-y-1">
                                    <p className="text-muted-foreground uppercase text-[9px] font-sans font-bold">Record ID</p>
                                    <p className="truncate">{table.id}</p>
                                </div>
                                <div className="space-y-1">
                                    <p className="text-muted-foreground uppercase text-[9px] font-sans font-bold">Organization ID</p>
                                    <p className="truncate">{table.organizationId}</p>
                                </div>
                                <div className="space-y-1">
                                    <p className="text-muted-foreground uppercase text-[9px] font-sans font-bold">Created By</p>
                                    <p className="truncate">{table.createdBy}</p>
                                </div>
                                <div className="space-y-1">
                                    <p className="text-muted-foreground uppercase text-[9px] font-sans font-bold">Created At</p>
                                    <p>{new Date(table.createdAt).toLocaleString()}</p>
                                </div>
                                <div className="space-y-1">
                                    <p className="text-muted-foreground uppercase text-[9px] font-sans font-bold">Updated At</p>
                                    <p>{new Date(table.updatedAt).toLocaleString()}</p>
                                </div>
                                <div className="space-y-1">
                                    <p className="text-muted-foreground uppercase text-[9px] font-sans font-bold">Version</p>
