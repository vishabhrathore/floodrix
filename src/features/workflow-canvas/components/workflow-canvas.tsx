...
const WorkflowCanvasInternal = observer(
  ({
    workflowId,
    workflowName,
    workflowStatus,
    ...
    useEffect(() => {
      store.flushHandler = async () => {
        ...
        try {
          ...
          runInAction(() => {
            store.isDirty = false;
            store.isPositionOnlyDirty = false;
    ...
    useEffect(() => {
      return () => {
        if (store.isDirty || store.isPositionOnlyDirty) store.flush();
      };
    }, [store]);
    ...
    useEffect(() => {
      ...
      return () =>
        window.removeEventListener("floodrix:configure-node", handler);
    }, [openConfigDrawer]);
