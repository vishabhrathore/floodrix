...
export function WorkflowFormRunner({
    ...
    useEffect(() => {
        if (pollData) {
            ...
            if (pollData.status === "COMPLETED" || pollData.status === "ERRORED" || pollData.status === "PAUSED") {
                setIsPolling(false);
            }
        }
        if (pollError) {
            setIsPolling(false);
            setState(s => ({ ...s, status: "ERRORED", error: { nodeId: "", message: pollError.message, type: "poll_error" } }));
        }
    }, [pollData, pollError]);
                                    ...
                                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                                        ...
                                        <tbody>
                                            {Object.entries(state.variables)
                                                ...
                                                .map(([key, value]) => (
                                                    <tr key={key}>
                                                        <td style={{ ...tdStyle, color: "#247ba0", fontWeight: 600 }}>{key}</td>
...
function BatchPanel({
    ...
    const startBatchMutation = useMutation(trpc.calcExecution.startRun.mutationOptions());
    ...
    const handleCalculateAll = async () => {
        ...
        for (const row of rows) {
            try {
                ...
                newResults.push({
                    ...row,
                    Q_result: (res.variables as any)?.Qd ?? (res.variables as any)?.Q_dicken ?? "N/A",
                    status: res.status
                });
            } catch (err) {
    ...
    return (
        <>
            <Card icon="📊" title="Batch Upload" sub={`${workflowName} — process multiple sites`} accent="teal">
            ...
            <Card icon="✏" title="Manual Data Entry" sub={`${rows.length} row${rows.length !== 1 ? "s" : ""}`} accent="red">
                                    ...
                                    {fields.map(f => (
                                        <td key={f.key} style={{ padding: "4px 6px" }}>
                                            <input
                                                ...
                                                style={{ width: 80, padding: "6px 8px", border: "1.5px solid #d1d5db", borderRadius: 6 }}
                                            />
                                        </td>
                                    ))}
                                    <td style={tdStyle}>
                                        {results[idx]?.Q_result ?? "—"}
                                    </td>
                                    <td style={{ padding: "4px 6px" }}>
                                        <button onClick={() => removeRow(idx)} style={{ color: "#d1d5db" }}>×</button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
                ...
                <div style={{ display: "flex", gap: 10, marginTop: 12 }}>
                    ...
                    <button onClick={handleCalculateAll} disabled={isCalculating} style={btnCalcStyle}>
    ...
    return (
        <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 12, marginBottom: 16, overflow: "hidden" }}>
            <div style={{ padding: "14px 20px", borderBottom: "1px solid #f3f4f6", display: "flex", alignItems: "center", gap: 10, background: "#fafbfc" }}>
                <div style={{ width: 32, height: 32, borderRadius: 9, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15, background: a.bg, color: a.color, flexShrink: 0 }}>
                    {icon}
                </div>
                <div>
...
function WaterwayCard({ label, value, unit, sub, best }: any) {
    return (
        <div style={{ border: `1.5px solid ${best ? "#0d9488" : "#e5e7eb"}`, borderRadius: 10, padding: "14px 15px", background: best ? "rgba(13,148,136,0.04)" : "#fff" }}>
            ...
            <span style={{ fontSize: 24, fontWeight: 800, color: best ? "#0d9488" : "#191919", fontFamily: "'Outfit', sans-serif" }}>{fmt(value, 2)}</span>
