...
import {
    Play, StepForward, X, RefreshCw, CheckCircle2,
    AlertTriangle, Loader2, SkipForward, ChevronRight,
} from "lucide-react";
    ...
    return (
        <Card className="border-purple-200 bg-purple-50/40 p-4">
            <div className="mb-3">
                ...
                <div className="mt-1 text-sm font-medium text-neutral-800">{field.nodeLabel}</div>
                {field.message && (
            ...
            <div className="space-y-3">
                {field.fields.map((f) => (
                    <div key={f.key}>
                        <Label htmlFor={f.key} className="text-xs font-medium text-neutral-700">
                            {f.label}
                            {f.unit && <span className="ml-1 text-neutral-400">({f.unit})</span>}
                            {f.required !== false && <span className="ml-0.5 text-red-500">*</span>}
                        </Label>
                        <Input
                            id={f.key}
...
function ValidationPauseView({ message, onCancel }: { message: string; onCancel: () => void }) {
    return (
        <Card className="border-amber-200 bg-amber-50/40 p-4">
            ...
            <Button size="sm" variant="outline" onClick={onCancel} className="mt-3 w-full">
                Cancel run
            </Button>
        </Card>
    );
...
function CompletedView({
    ...
    const visibleVars = Object.entries(variables).filter(([k]) => !k.startsWith("$"));
    ...
    return (
        <Card className="border-emerald-200 bg-emerald-50/40 p-4">
            ...
            <Button size="sm" onClick={onRunAgain} className="mt-4 w-full" variant="outline">
