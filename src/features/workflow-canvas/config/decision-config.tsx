...
export function DecisionConfig({ config, onSave }: DecisionConfigProps) {
  ...
  const [trueLabel, setTrueLabel] = useState(
    config.branches?.true?.label ?? "",
  );
  const [falseLabel, setFalseLabel] = useState(
    config.branches?.false?.label ?? "",
  );
  ...
  const doSave = useCallback(() => {
    onSave({
      ...
      branches: {
        true: {
          label: trueLabel,
          set_variables: config.branches?.true?.set_variables ?? {},
        },
        false: {
          label: falseLabel,
          set_variables: config.branches?.false?.set_variables ?? {},
        },
      },
    });
  ...
  return (
    <div>
      <ConfigSection
        title="Condition"
        description="mathjs expression that evaluates to true or false"
      >
        <ConfigField
          label="Condition Expression"
          hint="e.g. M > 25 or Q_dicken > 100"
        >
          <Textarea
            value={condition}
