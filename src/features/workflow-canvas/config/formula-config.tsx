...
import { Save, BookOpen, Code2, AlertTriangle, ExternalLink, } from "lucide-react";
...
export function FormulaConfig({ config, availableVariables = [], onSave }: FormulaConfigProps) {
    ...
    const [registryInputs, setRegistryInputs] = useState<RegistryVariable[]>(config.registry_inputs ?? []);
    const [registryOutput, setRegistryOutput] = useState<RegistryVariable | undefined>(config.registry_output);
    ...
    }, [source, expression, displayExpr, resultVar, resultUnit, precision, reference, effectiveBindings, requiredVars, availableVariables, registryId, registryInputs, registryOutput, onSave]);
            ...
            <div className="space-y-4">
                <Tabs value={source} onValueChange={(v) => setSource(v as "inline" | "registry")}>
                    ...
                    <TabsContent value="inline">
                        <ConfigSection title="Expression" description="Define custom engineering logic">
                            ...
                            <ConfigField label="Display Expression" hint="Pretty version for cards">
                                <Input
                                    value={displayExpr}
                    ...
                    <TabsContent value="registry">
                        <ConfigSection title="Registry Selection" description="Pick standard engineering formulas">
                            <ConfigField label="Formula Registry Item">
                                <FormulaSelector
                                    ...
                                    onSelect={(formula) => {
                                        ...
                                        if (!resultVar || resultVar === "undefined" || resultVar === "") {
                            ...
                            {registryId && (
                                <div className="mt-4 rounded-lg border border-slate-200 bg-slate-50 p-3">
                                    <div className="flex items-center justify-between mb-2">
                                        <h4 className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Active Formula</h4>
                                        <Badge variant="outline" className="h-4 text-[9px] bg-white">ID: {registryId.slice(0, 8)}</Badge>
                                    </div>
                                    <div className="space-y-2">
                                        ...
                                        <Button
                                            ...
                                            onClick={() => window.open(`/registery/formula/${registryId}`, '_blank')}
                                        >
                    ...
                    >
                        <div className="space-y-2.5">
                            {requiredVars.map((varName) => {
                                ...
                                const isBound = !!bound;
                                const regVar = registryInputs.find(ri => ri.notation === varName);

                                return (
                                    <div key={varName} className="rounded-lg border border-slate-200 bg-white p-2.5 shadow-sm">
                                        <div className="flex items-center gap-3 mb-2">
                                            <div className="flex flex-col flex-1 min-w-0">
                                                ...
                                                {regVar?.unit && (
                                                    <span className="text-[9px] text-slate-400 mt-0.5">Unit: {regVar.unit}</span>
                                                )}
                                            </div>
                                            {!isBound && <Badge variant="outline" className="h-4 px-1 text-[8px] border-amber-200 text-amber-600">Required</Badge>}
                                        </div>

                                        <Select
                                            value={bound ?? "__unbound__"}
                                            onValueChange={(v) => updateBinding(varName, v === "__unbound__" ? "" : v)}
                                        >
                                            <SelectTrigger className={cn("h-8 text-[11px]", !isBound && "border-amber-200 bg-amber-50/50")}>
                                                <SelectValue placeholder="Link to output..." />
                                            </SelectTrigger>
                                            <SelectContent>
                                                <SelectItem value="__unbound__" className="text-[11px] text-slate-400">Not linked</SelectItem>
                                                {availableVariables.map((av) => (
                                                    <SelectItem key={av.key} value={av.key} className="text-[11px]">
                                                        <span className="font-mono font-bold mr-2">{av.key}</span>
                ...
                <ConfigSection title="Output Result" description="Define the key for the computed value">
                    <div className="grid grid-cols-2 gap-3">
                        <ConfigField label={<span>Result Key <span className="text-red-500">*</span></span>} hint="Variable name">
                            <Input
                                value={resultVar}
                                onChange={(e) => {
                                ...
                                placeholder="e.g. Q"
                                className={cn("h-8 font-mono text-xs uppercase", (!resultVar || resultVar === "undefined") && "border-red-300 bg-red-50")}
                            />
                        </ConfigField>
