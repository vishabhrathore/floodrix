...
export function LookupConfig({ config, onSave }: LookupConfigProps) {
  ...
  const [inputKeys, setInputKeys] = useState<
    { key: string; type: "exact" | "range"; unit?: string }[]
  >(
    config.input_keys ?? [
      { key: "", type: "exact", unit: "" },
      { key: "", type: "range", unit: "" },
  ...
  const [multiRows, setMultiRows] = useState<MultiKeyRow[]>(
    config.multi_rows ?? [],
  );
      ...
      <Tabs value={mode} onValueChange={(v) => setMode(v as TableMode)}>
        ...
        <TabsContent value="interpolated" className="mt-0">
          <ConfigSection title="Interpolation Method">
            ...
            <ConfigField label="Extrapolation (outside data range)">
              <Select value={extrapolation} onValueChange={setExtrapolation}>
                ...
                <SelectContent>
                  <SelectItem value="clamp" className="text-xs">
                    Clamp to nearest point
                  </SelectItem>
                  <SelectItem value="extend" className="text-xs">
                    Extend linearly
                  </SelectItem>
          ...
          <div className="rounded-md border border-slate-200 bg-slate-50 p-3 text-[11px] text-slate-500">
            Interpolated values are computed at runtime between the nearest data
            points using the selected method. The result is written to{" "}
        ...
        <TabsContent value="multi_key" className="mt-0">
          ...
          <div className="rounded-md border border-slate-200 bg-slate-50 p-3 text-[11px] text-slate-500">
            ...
            <span className="font-mono font-semibold">exact match</span>, second
            key does{" "}
            <span className="font-mono font-semibold">range match</span>. First
            row where all keys match is selected.
          </div>
        </TabsContent>
      </Tabs>
        ...
        <div className="flex items-center justify-between rounded-lg border border-slate-200 px-3 py-2.5">
          <div>
            ...
            <p className="text-[10px] text-slate-400">
              Let user manually pick a different row at runtime
            </p>
          </div>
...
function RangeTableEditor({
  ...
  const addRow = () => {
    ...
    onUpdate([
      ...rows,
      { from: lastTo, to: null, value: 0, label: "", remarks: "" },
    ]);
  };
    ...
    <div>
      ...
      <div className="space-y-1">
        {rows.map((row, idx) => (
          <div
            key={idx}
            className="grid grid-cols-[60px_60px_1fr_60px_28px] items-center gap-1"
          >
            <Input
              type="number"
              step="any"
              value={row.from}
              onChange={(e) =>
                updateRow(idx, { from: parseFloat(e.target.value) || 0 })
              }
    ...
    <div>
        ...
        {sorted.map((pt, idx) => (
          ...
          >
            <Input
              ...
              value={pt.x}
              onChange={(e) =>
                updatePoint(idx, { x: parseFloat(e.target.value) || 0 })
              }
              className="h-7 font-mono text-[10px]"
            />
            <Input
              ...
              value={pt.y}
              onChange={(e) =>
                updatePoint(idx, { y: parseFloat(e.target.value) || 0 })
              }
              className="h-7 font-mono text-[10px]"
            />
            <Button
      ...
      {sorted.length >= 2 && (
        <div className="mt-2 text-[10px] text-slate-400 text-center">
          {sorted.length} points · x range: [{sorted[0].x} →{" "}
...
function MultiKeyInputs({
  ...
  onUpdate: (
    keys: { key: string; type: "exact" | "range"; unit?: string }[],
  ) => void;
}) {
  const updateKey = (
    idx: number,
    patch: Partial<{ key: string; type: "exact" | "range"; unit?: string }>,
  ) => {
  ...
  return (
    <div className="space-y-2">
      {inputKeys.map((k, idx) => (
        <div
          key={idx}
          className="grid grid-cols-[1fr_80px_60px_28px] items-end gap-1.5"
        >
          ...
          <ConfigField label="Match">
            <Select
              value={k.type}
              onValueChange={(v) =>
                updateKey(idx, { type: v as "exact" | "range" })
              }
            >
...
function MultiKeyTableEditor({
  ...
  const addRow = () => {
    ...
    for (const k of inputKeys)
      keys[k.key || `key_${inputKeys.indexOf(k)}`] = "";
  ...
  return (
    <div className="mt-3">
      ...
      <div
        ...
        style={{
          gridTemplateColumns: `${keyNames.map(() => "1fr").join(" ")} 70px 28px`,
        }}
      >
        {keyNames.map((k) => (
          <span
            key={k}
            className="text-[9px] font-bold uppercase tracking-wider text-violet-500"
          >
            {k}
          </span>
        ))}
        ...
        {rows.map((row, idx) => (
          ...
          >
            ...
            <Input
              ...
              value={row.value}
              onChange={(e) =>
                updateValue(idx, parseFloat(e.target.value) || 0)
              }
              className="h-7 font-mono text-[10px] font-bold"
            />
            <Button
