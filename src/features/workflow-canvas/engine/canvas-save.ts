...
export async function saveCanvasOptimized(input: CanvasSaveInput) {
  ...
  for (const rfNode of rfNodes) {
    ...
    if (
      Math.abs(old.positionX - rfNode.positionX) > 1 ||
      Math.abs(old.positionY - rfNode.positionY) > 1
    ) {
      movedNodes.push({
        id: rfNode.id,
        label: rfNode.label,
  ...
  const hasChanges =
    ...
    0;
  if (hasChanges) {
    await auditService.logNodeChanges(
      db,
