...
// ═══════════════════════════════════════════════════════════════════════════
import { type MathJsInstance, type MathNode, all, create, parse } from "mathjs";
...
export function validateExpression(expression: string): ValidationResult {
  ...
  if (expression.includes("import(") || expression.includes("require(")) {
    return {
      valid: false,
      errors: ["Module imports are not allowed"],
      warnings,
      variables,
      functions,
      complexity,
  ...
  if (complexity > MAX_COMPLEXITY) {
    errors.push(
      `Expression too complex (${complexity} nodes, max ${MAX_COMPLEXITY})`,
    );
  }
...
export function validateMultiLine(
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!match) {
      errors.push(
        `Line ${i + 1}: Expected "variable = expression", got: "${line}"`,
      );
      continue;
    }
    ...
    result.variables.forEach((v) => {
      if (!assigned.has(v.name))
        variables.push({ name: v.name, usage: "read" });
    });
...
export function extractVariables(expression: string): string[] {
  return validateExpression(expression)
    .variables.filter((v) => v.usage === "read")
    .map((v) => v.name);
}
...
export function safeEvaluate(
  ...
  const options: EvaluateOptions =
    typeof opts === "number" ? { precision: opts } : opts;
...
export function safeEvaluateMultiLine(
  ...
  for (const line of lines) {
    ...
    if (!match) {
      throw new Error(
        `Invalid line: "${line}". Expected format: variable = expression`,
      );
    }
