...
export function resolveExecutionOrder(
  ...
  const pairs: [string, string][] = edges.map((e) => [
    e.sourceNodeId,
    e.targetNodeId,
  ]);
...
export class WorkflowExecutor {
    ...
    await this.db.calcSession.update({
      ...
      data: {
        ...
        inputSnapshot: (session.inputSnapshot
          ? {
              ...(session.inputSnapshot as Record<string, unknown>),
              ...userInput,
            }
  ...
  async continueExecution(sessionId: string): Promise<ExecutionResult> {
      ...
      try {
        ...
        if (node.type === "INPUT") {
          ...
          if (shouldPause) {
            ...
            const allProvided = fields.every(
              (f) => variables[f.key] !== undefined,
            );
    ...
    await this.db.calcSession.update({
      ...
      data: {
        ...
        duration:
          endTime.getTime() -
          (session.startedAt?.getTime() ?? endTime.getTime()),
      },
    });
  ...
  private async executeFormula(
    ...
    if (config.source === "registry" && config.registry_id) {
      ...
      evalScope = {};
      for (const inputVar of registry.inputVariables as {
        notation: string;
        key: string;
      }[]) {
      ...
      const outVar = registry.outputVariable as {
        notation: string;
        key: string;
      };
    ...
    const precision =
      (config.overrides?.result_precision as number) ??
      config.result_precision ??
      3;
    const value =
  ...
  private async executeLookup(
    ...
    if (!matchedRow) {
      ...
      if (fallback === "first") {
        matchedRow = data[0] as Row;
        matchedIndex = 0;
      }
      if (fallback === "last") {
        matchedRow = data[data.length - 1] as Row;
        matchedIndex = data.length - 1;
      }
    }
  ...
  private async executeInterpolation(
    ...
    if (xValue === undefined) {
      throw new Error(
        `Interpolation input "${inputVar}" not found in variables`,
      );
    }
  ...
  private executeDecision(
    ...
    const varsSet = (branches[branchTaken]?.set_variables ?? {}) as Record<
      string,
      unknown
    >;
  ...
  private executeDisplay(config: NodeConfig, variables: VariableContext) {
    ...
    if (rule === "max") {
      ...
      selectedValue = Math.min(
        ...values.filter((v) => v.value > 0).map((v) => v.value),
      );
