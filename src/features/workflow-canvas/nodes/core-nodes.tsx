...
function FormulaNodeInner({ id, data, selected }: NodeProps) {
  ...
  const displayExpr = (config.display_expression ||
    config.expression ||
    "") as string;
  ...
  const showVars = (config.showVars || []) as {
    key: string;
    label: string;
    unit: string;
  }[];
...
function GraphInterpolationNodeInner({ id, data, selected }: NodeProps) {
  ...
  return (
    <BaseNode
      ...
      subtitle={
        method ? `${method} · ${points.length} pts` : `${points.length} points`
      }
      selected={selected}
      executionStatus={d.executionStatus}
      reference={reference}
      ...
      {hasCurve ? (
        <>
          ...
          <FieldRow label="Input">
            {inputVar ? <VariablePill name={inputVar} /> : "—"}
          </FieldRow>

          {interpolatedValue !== undefined && resultVar && (
            <ResultDisplay
              label={resultVar}
              value={interpolatedValue.toFixed(4)}
              accent="#db2777"
            />
          )}
        </>
...
function DecisionNodeInner({ id, data, selected }: NodeProps) {
  ...
  const branches = config.branches as
    | Record<string, { label: string; set_variables?: Record<string, unknown> }>
    | undefined;
            ...
            <div style={{ display: "flex", gap: 6, marginTop: 6 }}>
              ...
              <div
                style={{
                  ...
                  backgroundColor:
                    conditionResult === true ? "#f0fdf4" : "transparent",
                  fontSize: 10,
                  fontWeight: 500,
                }}
              >
                <div
                  style={{ color: "#16a34a", fontWeight: 600, marginBottom: 1 }}
                >
                  True
                </div>
                <div
              ...
              <div
                style={{
                  ...
                  backgroundColor:
                    conditionResult === false ? "#fef2f2" : "transparent",
                  fontSize: 10,
                  fontWeight: 500,
                }}
              >
                <div
                  style={{ color: "#dc2626", fontWeight: 600, marginBottom: 1 }}
                >
                  False
                </div>
                <div
...
function DisplayNodeInner({ id, data, selected }: NodeProps) {
  ...
  const compareVars = (config.compare_variables || []) as {
    key: string;
    method?: string;
    label?: string;
  }[];
  ...
  const ranking = result?.ranking as
    | { key: string; method: string; value: number }[]
    | undefined;
  ...
  return (
    <BaseNode
      ...
      subtitle={
        mode === "comparison" ? `Compare ${compareVars.length} methods` : mode
      }
      selected={selected}
      executionStatus={d.executionStatus}
      width={280}
              ...
              {ranking.slice(0, 4).map((r, i) => (
                <div
                  ...
                  style={{
                    ...
                    border:
                      i === 0 ? "1px solid #fde68a" : "1px solid transparent",
                    fontSize: 11,
                  }}
                >
                  <span
