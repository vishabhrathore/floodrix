...
const CATEGORIES: PaletteCategory[] = [
  {
    label: "Data Input",
    nodes: [{ type: "INPUT", available: true }],
  },
  {
    label: "Computation",
    nodes: [
      ...
      <div style={{ overflow: "auto", flex: 1, padding: "4px 0" }}>
        {filteredCategories.map((cat) => (
          <div key={cat.label}>
            ...
            <button
              onClick={() =>
                setExpanded((e) => ({ ...e, [cat.label]: !e[cat.label] }))
              }
              style={{
                display: "flex",
                alignItems: "center",
...
function PaletteItem({
  ...
  return (
    <div
      ...
      onMouseEnter={(e) => {
        if (available) e.currentTarget.style.background = "#f9fafb";
      }}
      onMouseLeave={(e) => {
    ...
    >
      ...
      >
        {IconComponent && (
          <IconComponent size={12} color={accent.accent} strokeWidth={2} />
        )}
      </div>
