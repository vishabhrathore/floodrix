...
export function WorkspaceCanvasEditor({
    ...
    const handleSave = async () => {
        ...
        const create = dbNodes.filter(n => n._isNew && !n._isDeleted).map(n => ({
            id: n.id,
            parentId: n.parentId!,
            nodeType: n.nodeType,
            name: n.name,
        ...
        const update = dbNodes.filter(n => n._isDirty && !n._isNew && !n._isDeleted).map(n => ({
            id: n.id,
            data: {
                parentId: n.parentId,
                name: n.name,
        ...
        const deleteIds = dbNodes.filter(n => n._isDeleted && !n._isNew).map(n => n.id);
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20 backdrop-blur-sm">
                    <div className="bg-white rounded-xl shadow-2xl w-[400px] max-h-[500px] flex flex-col border border-gray-100 overflow-hidden animate-in fade-in zoom-in duration-200">
                        <div className="p-4 border-b border-gray-50 flex items-center justify-between bg-gray-50/50">
                            <div>
                                <h2 className="text-sm font-bold text-gray-800">Select Workflow to Link</h2>
                                <p className="text-[10px] text-gray-500">
                                    {searchAllOrgs ? "Searching across all organizations" : `In ${orgData?.name || "Organization"}`}
                                </p>
                            </div>
                            <button
                        ...
                        <div className="flex-1 overflow-y-auto p-2 bg-white">
                            {workflowsLoading ? (
                                <div className="flex items-center justify-center py-10 text-gray-400 text-xs">
                                    <Loader2 size={16} className="animate-spin mr-2" /> Loading workflows...
                                </div>
