...
function TreeNode({
  ...
  const linkCount = children.filter(
    (c) => c.nodeType === "WORKFLOW_LINK",
  ).length;
          ...
          {isEditing ? (
            ...
            <span
              className={`text-[11px] font-medium truncate block ${
                status === "coming_soon" ? "text-gray-300" : "text-gray-700"
              }`}
            >
                ...
                {isFolder && (
                  <>
                    ...
                    <CtxButton
                      ...
                      label="Add link"
                      onClick={() => {
                        createWorkflowLink(node.id, "New link", "");
                        setMenuOpen(false);
                      }}
                    />
...
function CtxButton({
  ...
  return (
    <button
      onClick={onClick}
      className={`flex items-center gap-2 w-full px-2 py-1.5 text-[11px] font-medium rounded-md transition-colors ${
        danger
