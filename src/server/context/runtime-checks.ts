import { PrismaClient, BillingType } from "@/generated/prisma";
...
export async function assertAndConsumeBillingQuota(
    ...
    if (!ctx.billing) {
        throw new TRPCError({ code: "FORBIDDEN", message: "No active billing plan" });
    }
    ...
    await db.$transaction(async (tx) => {
        ...
        if (billing.billingType === BillingType.SUBSCRIPTION) {
            ...
            if (updated.count === 0) {
                throw new TRPCError({ code: "FORBIDDEN", message: "Monthly run limit exceeded" });
            }
        }
