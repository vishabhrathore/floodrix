...
import { emptySessionMetadata, METADATA_VERSION } from "./types";

// ─── Result types ─────────────────────────────────────────────────────────

export interface LoadedWorkflow {
    workflow: Pick<CalcWorkflow, "id" | "name" | "organizationId" | "metadata">;
    nodes: Pick<CalcNode, "id" | "type" | "label" | "description" | "config" | "sortOrder">[];
    edges: Pick<CalcEdge, "id" | "sourceNodeId" | "targetNodeId" | "sourceHandle" | "targetHandle" | "condition">[];
    variables: Pick<CalcVariable, "contextKey" | "defaultValue" | "unit" | "dataType">[];
}
...
export class SessionRepository {
        ...
        const workflow = await this.db.calcWorkflow.findUniqueOrThrow({
            ...
            select: {
                ...
                nodes: {
                    ...
                    select: { id: true, type: true, label: true, description: true, config: true, sortOrder: true },
                    orderBy: { sortOrder: "asc" },
                },
                edges: {
                    where: { deletedAt: null },
                    select: { id: true, sourceNodeId: true, targetNodeId: true, sourceHandle: true, targetHandle: true, condition: true },
                },
                variables: {
                    where: { deletedAt: null },
                    select: { contextKey: true, defaultValue: true, unit: true, dataType: true },
                },
            },
        });
     ...
     */
    async findByIdempotencyKey(workflowId: string, idempotencyKey: string): Promise<LoadedSession | null> {
    ...
    async updateProgress(sessionId: string, variables: VariableSnapshot, currentIndex: number): Promise<void> {
    ...
    async createPendingNodeExecutions(sessionId: string, executionOrder: string[]): Promise<void> {
     ...
     */
    async updateNodeRunning(sessionId: string, nodeId: string, inputVars: VariableMap): Promise<void> {
    ...
    async updateNodeCompleted(input: {
        ...
        await this.db.calcNodeExecution.updateMany({
            where: { sessionId: input.sessionId, calcNodeId: input.nodeId, status: "RUNNING" },
            data: {
                status: "COMPLETED",
    ...
    async updateNodeSkipped(sessionId: string, nodeId: string): Promise<void> {
        ...
        await this.db.calcNodeExecution.updateMany({
            where: { sessionId, calcNodeId: nodeId, status: { in: ["PENDING", "RUNNING"] } },
            data: {
                status: "SKIPPED",
                completedAt: this.clock.nowDate(),
            },
    ...
    async insertNodeExecutionLog(
        ...
        await this.db.calcNodeExecution.createMany({
            data: entries.map((e) => ({
                ...
                status: e.status,
                inputVars: e.inputVars ? (e.inputVars as Prisma.InputJsonValue) : undefined,
                outputVars: e.outputVars ? (e.outputVars as Prisma.InputJsonValue) : undefined,
    ...
    private hydrateSession(row: {
        ...
        return {
            ...
            metadata,
            inputSnapshot: (row.inputSnapshot ?? null) as Record<string, unknown> | null,
...
function hydrateMetadata(raw: unknown): SessionMetadata {
    ...
    if (version === METADATA_VERSION) {
        return {
            metadataVersion: METADATA_VERSION,
            stepPauseReason: (m.stepPauseReason as SessionMetadata["stepPauseReason"]) ?? null,
            skippedNodes: Array.isArray(m.skippedNodes) ? (m.skippedNodes as string[]) : [],
            stepMode: !!m.stepMode,
