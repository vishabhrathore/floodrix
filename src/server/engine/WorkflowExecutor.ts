...
import { SessionRepository, type LoadedWorkflow, type LoadedSession } from "./SessionRepository";
...
import { createRegistryResolver, type RegistryResolver } from "@/features/workflow-canvas/engine/registry-resolver";
...
export class WorkflowExecutor {
    ...
    async startExecution(
        ...
        if (idempotencyKey) {
            const existing = await this.deps.repo.findByIdempotencyKey(calcWorkflowId, idempotencyKey);
        ...
        for (const v of wf.variables) {
            if (v.defaultValue !== null) variables[v.contextKey] = v.defaultValue as never;
        }
        Object.assign(variables, initialValues);

        const session = await this.deps.repo.createSession({
            calcWorkflowId, actorId, executionOrder,
        ...
        if (options.liveUpdates) {
            await this.deps.repo.createPendingNodeExecutions(session.id, executionOrder);
        }
    ...
    async resumeWithInput(
        ...
        if (session.currentNodeId !== nodeId) {
            throw new Error(`Session is paused at node ${session.currentNodeId}, not ${nodeId}`);
        }

        const variables: VariableSnapshot = { ...session.variables, ...userInput } as VariableSnapshot;
        ...
        await this.deps.repo.updateNodeCompleted({
            ...
            result: { userInput, providedAt: this.deps.clock.nowDate().toISOString() },
            durationMs: 0,
        });

        await this.deps.repo.resumeSession({
            sessionId, userInput, nextIndex: currentIdx + 1, variables,
        });
     ...
     */
    async stepForward(sessionId: string, options: ExecutionOptions = {}): Promise<ExecutionResult> {
        const session = await this.deps.repo.loadSession(sessionId);
        if (session.status !== "PAUSED") {
            throw new Error(`stepForward: session ${sessionId} is not paused (status: ${session.status})`);
        }
    ...
    async stepBack(sessionId: string, targetNodeId: string): Promise<ExecutionResult> {
        const session = await this.deps.repo.loadSession(sessionId);
        if (session.status !== "PAUSED") {
            throw new Error(`stepBack: session ${sessionId} is not paused (status: ${session.status})`);
        }
    ...
    async continueExecution(sessionId: string, options: ExecutionOptions = {}): Promise<ExecutionResult> {
        const session = await this.deps.repo.loadSession(sessionId);
        const wf = await this.deps.repo.loadWorkflow(session.calcWorkflowId);
        ...
        try { await registry.prefetchForWorkflow(this.deps.db, session.calcWorkflowId); }
        ...
        while (currentIndex < session.executionOrder.length) {
            ...
            if (skipSet.has(nodeId)) {
                await this.deps.emitter.emit({
                    type: "node:skipped", sessionId, nodeId, reason: "decision_branch",
                });
                currentIndex++;
                continue;
            }
            ...
            if (isStructuralNodeType(node.type)) {
                await this.deps.emitter.emit({
                    type: "node:skipped", sessionId, nodeId, reason: "structural",
                });
                currentIndex++;
                continue;
            }
            ...
            if (isAsyncNodeType(node.type) && !options.isBackgroundRun) {
                return this.errorOut(sessionId, session, wf, node, currentIndex, variableStore,
                    new Error(`Async node type ${node.type} requires Chunk 4 (Inngest) to execute`));
            }

            const handler = this.deps.registry.get(node.type);
            if (!handler) {
                await this.deps.emitter.emit({
                    type: "node:skipped", sessionId, nodeId, reason: "no_handler",
                });
                currentIndex++;
                continue;
            }
            ...
            await this.deps.emitter.emit({
                type: "node:started", sessionId, nodeId,
                nodeLabel: node.label, nodeType: node.type,
            ...
            const ctx: ExecutionContext = {
                ...
                db: this.deps.db, registry,
                sessionId, workflowId: session.calcWorkflowId,
            ...
            if (outcome.kind === "errored") {
                await this.deps.emitter.emit({
                    type: "node:errored", sessionId, nodeId, nodeLabel: node.label,
                    error: outcome.error,
                    errorType: classifyError(outcome.error),
                    durationMs,
                });
                return this.errorOut(sessionId, session, wf, node, currentIndex, variableStore, outcome.error);
            }
            ...
            if (outcome.kind === "paused") {
                await this.deps.emitter.emit({
                    type: "node:waiting", sessionId, nodeId, nodeLabel: node.label,
                    pauseReason: outcome.reason,
                });
                await this.deps.emitter.emit({
                    type: "session:paused", sessionId, workflowId: session.calcWorkflowId,
                    nodeId, pauseReason: outcome.reason,
                    skippedNodes: [...skipSet], stepMode,
                });
                await this.deps.repo.pauseSession({
                    sessionId, nodeId, currentIndex,
                ...
                return {
                    ...
                    pausedNode: outcome.fields ? {
                        nodeId, nodeLabel: outcome.nodeLabel ?? node.label,
                        fields: outcome.fields, message: outcome.pauseMessage,
                    } : null,
                };
            }
            ...
            await this.deps.emitter.emit({
                type: "node:completed", sessionId, nodeId, nodeLabel: node.label,
                nodeType: node.type, stepNumber: currentIndex,
                outputs: outcome.outputs, result: outcome.result, durationMs,
            });

            await this.deps.repo.updateProgress(sessionId, variableStore.snapshot(), currentIndex + 1);

            // ── CHUNK 3: stepMode pause after each completed node ────
            if (stepMode) {
                const stepOutput: StepOutput = {
                    nodeId, nodeLabel: node.label, nodeType: node.type,
                    outputs: outcome.outputs, result: outcome.result,
                    durationMs,
                    stepNumber: currentIndex,
                    totalSteps,
                };
                ...
                await this.deps.emitter.emit({
                    type: "session:paused", sessionId, workflowId: session.calcWorkflowId,
                    nodeId, pauseReason: "step_complete",
        ...
        const { durationMs } = await this.deps.repo.completeSession({
            sessionId, variables: finalSnapshot,
        });
    ...
    private async runHandlerWithTimeout(handler: NodeHandler, ctx: ExecutionContext) {
        if (!handler.timeoutMs) return handler.execute(ctx);

        let timer: ReturnType<typeof setTimeout> | null = null;
        const timeoutPromise = new Promise<never>((_, reject) => {
            timer = setTimeout(() => {
                reject(new Error(`Handler ${handler.type} timed out after ${handler.timeoutMs}ms`));
            }, handler.timeoutMs);
        });
        ...
        try {
            ...
            return { kind: "errored" as const, error: err instanceof Error ? err : new Error(String(err)) };
        } finally {
...
function resolveExecutionOrder(wf: LoadedWorkflow): string[] {
    ...
    const pairs: [string, string][] = wf.edges.map((e) => [e.sourceNodeId, e.targetNodeId]);
    try {
        return toposort.array(allNodeIds, pairs);
    } catch (err) {
        if ((err as Error).message?.includes("cycle")) {
            throw new Error("Circular dependency detected in workflow. Check for loops in your node connections.");
        }
        throw err;
    }
}