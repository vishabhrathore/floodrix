...
export class CustomCodeHandler implements NodeHandler {
    ...
    async execute(ctx: ExecutionContext): Promise<NodeOutcome> {
        try {
            ...
            if (!code) {
                return toErroredOutcome(new Error("CUSTOM_CODE has no code configured"));
            }
            if (outputVarNames.length === 0) {
                return toErroredOutcome(new Error("CUSTOM_CODE has no output_variables declared"));
            }
            ...
            ctx.variables.trackNodeOutput(ctx.node.id, ctx.node.label, trackedOutputs);
