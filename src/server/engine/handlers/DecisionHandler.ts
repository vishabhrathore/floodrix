    ...
    async execute(ctx: ExecutionContext): Promise<NodeOutcome> {
        try {
            ...
            if (!config.condition?.trim()) {
                return toErroredOutcome(new Error("DECISION has no condition configured"));
            }
            ...
            const setVars = (branches[branchTaken]?.set_variables ?? {}) as VariableMap;
...
function computeSkipSet(
    ...
    const notTakenSeeds = edges
        .filter((e) => e.sourceNodeId === decisionNodeId && e.sourceHandle === notTakenHandle)
