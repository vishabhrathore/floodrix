    ...
    try {
      ...
      if (compareVars.length === 0) {
        return toErroredOutcome(
          new Error("DISPLAY has no variables to compare"),
        );
      }
      ...
      if (rule === "max") {
        ...
        if (positiveValues.length === 0) {
          return toErroredOutcome(
            new Error("DISPLAY (min): no positive values to compare"),
          );
        }
        ...
        if (positiveValues.length === 0) {
          return toErroredOutcome(
            new Error("DISPLAY (average): no positive values to compare"),
          );
        }
        selectedValue =
          positiveValues.reduce((s, v) => s + v.value, 0) /
        ...
        if (!config.custom_selection_expr) {
          return toErroredOutcome(
            new Error("DISPLAY (custom): no custom_selection_expr configured"),
          );
        }
        ...
        return toErroredOutcome(
          new Error(`DISPLAY: unknown selection_rule "${rule}"`),
        );
      }
