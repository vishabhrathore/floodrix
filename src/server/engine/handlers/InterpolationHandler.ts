    ...
    async execute(ctx: ExecutionContext): Promise<NodeOutcome> {
        try {
            ...
            if (config.source === "registry" && config.registry_id) {
                ...
                const interpConfig = registry.interpolationConfig as { method?: string; extrapolation?: string } | null;
                method = config.overrides?.interpolation_method ?? interpConfig?.method ?? "linear";
            ...
            if (!inputVar) {
                return toErroredOutcome(new Error("GRAPH_INTERPOLATION has no input variable configured"));
            }
            if (points.length === 0) {
                return toErroredOutcome(new Error("GRAPH_INTERPOLATION has no data points"));
            }

            const xRaw = ctx.variables.get(inputVar);
            if (xRaw === undefined) {
                return toErroredOutcome(new Error(`Interpolation input "${inputVar}" not found in variables`));
            }
            const xValue = Number(xRaw);
            if (!Number.isFinite(xValue)) {
                return toErroredOutcome(new Error(`Interpolation input "${inputVar}" is not a finite number: ${xRaw}`));
            }
