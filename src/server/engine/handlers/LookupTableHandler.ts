    ...
    try {
      ...
      if (inputKeys.length === 0 || inputKeys[0] === "") {
        return toErroredOutcome(
          new Error("LOOKUP_TABLE has no lookup key configured"),
        );
      }
      ...
      for (const k of inputKeys) {
        ...
        if (v === undefined) {
          return toErroredOutcome(
            new Error(`Lookup key "${k}" not found in variables`),
          );
        }
        inputValues[k] = v;
      }
      ...
      if (!finalRow) {
        return toErroredOutcome(
          new Error("Lookup fallback failed to produce a row"),
        );
      }
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (matchMode === "range") {
      const range =
        r.range ??
        (r.from !== undefined
          ? ([r.from, r.to ?? null] as [number, number | null])
          : null);
      if (!range) continue;
      ...
      if (r.key === value || String(r.key) === String(value))
