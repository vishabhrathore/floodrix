    ...
    try {
      ...
      if (!inputVar)
        return toErroredOutcome(
          new Error("UNIT_CONVERSION: input_variable not configured"),
        );
      if (!outputVar)
        return toErroredOutcome(
          new Error("UNIT_CONVERSION: output_variable not configured"),
        );
      ...
      if (inputRaw === undefined) {
        return toErroredOutcome(
          new Error(
            `UNIT_CONVERSION: input "${inputVar}" not found in variables`,
          ),
        );
      }
      ...
      if (!Number.isFinite(inputVal)) {
        return toErroredOutcome(
          new Error(
            `UNIT_CONVERSION: input "${inputVar}" is not a finite number`,
          ),
        );
      }
