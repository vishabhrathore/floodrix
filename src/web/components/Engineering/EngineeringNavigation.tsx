    ...
    <nav className="mt-4 px-4 md:px-12 lg:px-20 max-w-[1440px] mx-auto overflow-x-auto no-scrollbar">
      <div className="flex gap-6 md:gap-12 border-b border-slate-200 min-w-max">
        {Object.entries(tree).map(([id, item]) => (
          ...
          >
            <div
              className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-500 ${currentModule === id && mode === "manual" ? "bg-brand-red/5 text-brand-red" : "bg-slate-50 text-slate-300 group-hover:bg-slate-100"}`}
            >
