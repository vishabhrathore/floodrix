...
const EngineeringPlatform: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#fcfcfc] pb-32 selection:bg-brand-red selection:text-white">
      ...
      {/* Hero Section of the Platform */}
      <section className="bg-brand-dark pt-48 pb-32 px-6 md:px-20 lg:px-32 relative overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-12 mb-20">
            <motion.div 
              ...
              className="max-w-3xl"
            >
              <span className="text-[10px] font-bold text-brand-red uppercase tracking-[0.5em] mb-8 block font-mono">
                ENGINEERING CORE v4.2.0
              </span>
              <h1 className="text-5xl md:text-6xl lg:text-7xl font-serif text-white leading-[1.1] font-medium tracking-tight">
            ...
            <motion.div 
              ...
              className="flex bg-white/5 p-2 rounded-2xl backdrop-blur-xl border border-white/10 self-start shadow-2xl"
            >
              <button 
                onClick={() => setMode('manual')}
                className={`px-10 py-3.5 rounded-xl text-[10px] font-bold uppercase tracking-[0.2em] transition-all duration-500 ${
                  mode === 'manual' ? 'bg-brand-red text-white shadow-[0_8px_20px_-5px_rgba(251,54,64,0.5)]' : 'text-white/40 hover:text-white'
                }`}
              >
                Manual
              </button>
              <button 
                ...
                className={`px-10 py-3.5 rounded-xl text-[10px] font-bold uppercase tracking-[0.2em] transition-all duration-500 ${
                  mode === 'batch' ? 'bg-brand-red text-white shadow-[0_8px_20px_-5px_rgba(251,54,64,0.5)]' : 'text-white/40 hover:text-white'
                }`}
              >
                Batch
              </button>
            ...
            {Object.entries(TREE).map(([id, item], idx) => (
              <motion.button
                ...
                className={`flex items-center gap-4 transition-all relative group h-full pb-2 ${
                  currentModule === id && mode === 'manual' ? 'text-white' : 'text-white/20 hover:text-white/60'
                }`}
              >
                <div className={`p-2.5 rounded-xl transition-all duration-500 ${
                  currentModule === id && mode === 'manual' ? 'bg-brand-red text-white scale-110 shadow-lg' : 'bg-white/5 text-white/30 group-hover:bg-white/10'
                }`}>
        ...
        <div className="lg:col-span-4 space-y-10">
           <div 
             ...
             className={`p-12 border-2 border-dashed rounded-[3rem] transition-all duration-700 cursor-pointer flex flex-col items-center justify-center text-center gap-6 group relative overflow-hidden ${
               isDragging ? 'border-brand-red bg-brand-red/[0.02]' : 'border-gray-200 hover:border-brand-red/30 bg-white hover:bg-[#fcfcfc]'
             }`}
           >
           ...
           <div className="bg-brand-dark rounded-[2.5rem] p-10 space-y-8 relative overflow-hidden ring-1 ring-white/5 shadow-2xl">
              ...
              <button 
                ...
                className={`w-full py-5 rounded-2xl font-black text-xs tracking-[0.5em] uppercase transition-all duration-700 relative z-10 flex items-center justify-center gap-4 group ${
                   batchFile ? 'bg-brand-red text-white shadow-lg hover:shadow-brand-red/40 hover:-translate-y-1' : 'bg-white/5 text-white/20 cursor-not-allowed'
                }`}
              >
