...
const Header: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const sections = [
      ...
      { id: "#storytelling", isDark: false },
      { id: "#projects", isDark: true },
    ...
    sections.forEach(section => {
      const trigger = ScrollTrigger.create({
        ...
        onToggle: (self) => {
          if (section.isDark) {
            setIsDarkSection(self.isActive);
          }
          if (section.isFooter) {
            setIsInFooter(self.isActive);
          }
        },
      });
  ...
  const navLinks = [
    ...
    { name: 'Expert Panel', href: '/team' },
    {
      ...
      dropdown: [
        ...
        { name: 'Highway Drainage', icon: <Droplet className="w-4 h-4" />, href: '/#services' },
        { name: 'Infrastructure Modelling', icon: <Waves className="w-4 h-4" />, href: '/#services' },
        { name: 'Groundwater Services', icon: <Map className="w-4 h-4" />, href: '/#services' },
      ]
    },
  ...
  return (
    <nav
        ...
        {isMenuOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed top-0 right-0 bottom-0 w-[85%] max-w-[400px] bg-brand-dark z-[70] md:hidden shadow-[-20px_0_40px_rgba(0,0,0,0.5)] border-l border-white/5 flex flex-col"
            >
      ...
      <div className={`absolute bottom-0 left-0 h-[1px] bg-gradient-to-r from-transparent via-brand-red to-transparent transition-all duration-1000 ${isScrolled || !isHome ? 'w-full opacity-40' : 'w-0 opacity-0'}`} />
    </nav>
  );
};
