...
const Header: React.FC = () => {
    ...
    const navShadow = isScrolled && !isDarkSection && !isInFooter ? 'shadow-xl' : '';
    ...
    // ── Dropdown background ───────────────────────────────────────────────────
    const dropdownBg = !isDarkSection && !isInFooter && isScrolled
        ? 'bg-white/80 backdrop-blur-2xl'
        : 'bg-brand-dark/90 backdrop-blur-3xl';
    const dropdownItemColor = !isDarkSection && !isInFooter && isScrolled
        ? 'text-gray-600 hover:bg-gray-50 hover:text-brand-red'
        : 'text-white/60 hover:bg-white/5 hover:text-white';
    const dropdownIconBg = !isDarkSection && !isInFooter && isScrolled
        ? 'bg-gray-100'
            ...
            >
                <div className="w-full px-4 md:px-12 lg:px-20 flex justify-between items-center">
                    ...
                    <Link href="/" className="flex items-center space-x-4 group">
                        <span className={`text-h3 font-serif font-bold transition-colors duration-300 ${textColor}`}>
                    ...
                    <div className="hidden md:flex items-center space-x-12">
                            ...
                            <div key={link.name} className="relative group">
                                ...
                                {link.dropdown && (
                                    <div className="absolute top-full left-0 pt-6 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-500 translate-y-4 group-hover:translate-y-0">
                                        <div className={`w-72 rounded-xl shadow-2xl overflow-hidden border border-white/10 ${dropdownBg}`}>
                                            <div className="p-3 grid grid-cols-1 gap-1">
                                                {link.dropdown.map((sub) => (
                                                    ...
                                                    >
                                                        <div className={`p-2 rounded-lg transition-colors ${dropdownIconBg}`}>
                        ...
                        <div className={`h-8 w-[1px] mx-2 ${whiteText ? 'bg-white/10' : 'bg-gray-200'}`} />
                    </div>

                    {/* Mobile toggle */}
                    <button className="md:hidden p-3" onClick={() => setIsMenuOpen(v => !v)}>
                        {isMenuOpen
                ...
                <div className="absolute bottom-0 inset-x-0 overflow-hidden h-[1px]">
                    <div className={`h-full bg-gradient-to-r from-transparent via-brand-red to-transparent transition-all duration-1000 ${isScrolled ? 'w-full opacity-40' : 'w-0 opacity-0'}`} />
                </div>
            </nav>
                    ...
                    <>
                        ...
                        >
                            <div className="p-8 flex items-center justify-between border-b border-white/5">
                                ...
                                <button className="p-2 text-white/50 hover:text-white transition-colors" onClick={() => setIsMenuOpen(false)}>
                                    <X className="w-6 h-6" />
                                </button>
                            </div>
