...
import { Droplets, Anchor, CloudRain, Mountain } from 'lucide-react';
...
export const FORMULAS: Record<string, FormulaDef> = {
    ...
    calc(r) {
      ...
      return {
        ...
        vars: [
          ...
          { k: 'Q', d: 'Q = C × M^(3/4)', v: fmt(Q), u: 'Cumecs', result: true }
        ],
    ...
    calc(r) {
      ...
      return {
        ...
        vars: [
          ...
          { k: 'Q', d: 'Q = C × M^(2/3)', v: fmt(Q), u: 'Cumecs', result: true }
        ],
  ...
  ingli: {
    ...
    calc(r) {
      ...
      return {
        ...
        vars: [
          { k: 'M', d: 'Catchment Area', v: r.M, u: 'Km²' },
          { k: '√(M+10)', d: 'Square root term', v: fmt(Math.sqrt(r.M + 10)), u: '' },
          { k: 'Q', d: 'Q = 125M / √(M+10)', v: fmt(Q), u: 'Cumecs', result: true }
        ],
    ...
    calc(r) {
      ...
      return {
        ...
        vars: [
          ...
          { k: 'Q', d: 'Q = C·(0.386A)^0.894·(0.386A)^(-0.048)', v: fmt(Q), u: 'Cumecs', result: true }
        ],
  ...
  fuller: {
    ...
    fields: [
      { key: 'M', label: 'Catchment Area', unit: 'Km²', def: 24.1 },
      { key: 'T_return', label: 'Return Period', unit: 'years', def: 100, hint: 'Design return period' },
    ],
    calc(r) {
      const Q = 1.88 * Math.pow(r.M, 0.8) * (1 + 0.8 * Math.log10(r.T_return));
      const pts = [2, 5, 10, 25, 50, 100, 200, 500].map(t => ({ x: t, y: 1.88 * Math.pow(r.M, 0.8) * (1 + 0.8 * Math.log10(t)) }));
      return {
        Q,
        vars: [
          { k: 'M', d: 'Catchment Area', v: r.M, u: 'Km²' },
          ...
          { k: 'Q', d: 'QTP = 1.88·M^0.8·(1+0.8·logT)', v: fmt(Q), u: 'Cumecs', result: true }
        ],
        chartPts: pts,
        chartX: r.T_return,
    ...
    calc(r) {
      ...
      return {
        ...
        vars: [
          ...
          { k: 'Q', d: 'Q = 0.028·P·f·A·Ic', v: fmt(Q), u: 'Cumecs', result: true }
        ],
        chartPts: pts,
        chartX: tc,
  ...
  terzaghi: { name: 'Terzaghi Bearing', expr: 'qult = cNc + qNq + 0.5γBNγ', ref: 'IS-6403', region: 'Geotechnical', stub: true, fields: [], calc: () => ({ Q: 0, vars: [], chartX: 0 }) },
  settlement: { name: 'Consolidation Settlement', expr: 'Sc = [Cc/(1+e0)] H log...', ref: 'Soil Mechanics', region: 'Geotechnical', stub: true, fields: [], calc: () => ({ Q: 0, vars: [], chartX: 0 }) },
};
...
export const TREE: Record<string, TreeNode> = {
  ...
  ground: {
    label: 'Ground Analysis', icon: <Mountain />,
    desc: 'Geotechnical assessment including bearing capacity and settlement',
    children: {
      bearing: {
        label: 'Bearing Capacity', icon: '🏗', desc: 'Terzaghi / IS-6403 bearing capacity', children: {
          terzaghi: { label: 'Terzaghi Method', icon: 'ƒ', formula: 'terzaghi', leaf: true, desc: '' },
        }
      },
      settlement: {
        label: 'Settlement', icon: '📉', desc: 'Primary consolidation settlement', children: {
          consolidation: { label: 'Consolidation', icon: 'ƒ', formula: 'settlement', leaf: true, desc: '' },
        }
      },
    }
  }
