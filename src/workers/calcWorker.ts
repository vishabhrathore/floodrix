...
import { redisConnection, calcQueue, addJob } from "@/lib/bullmq";
  ...
  new Worker(
    ...
    async (job: Job) => {
      ...
      if (type === "calc/session.start-background") {
         ...
         await addJob(calcQueue, "resume", { sessionId, reason: "background_batch", type: "calc/session.resume" });
         return { started: true };
      }

      if (type === "calc/session.resume") {
        console.log(`Resuming session ${sessionId} (reason: ${reason ?? "unspecified"})`);

        // Load session status
        const session = await prisma.calcSession.findUnique({
            where: { id: sessionId },
            select: { status: true, pauseReason: true, calcWorkflowId: true, actorId: true },
        });
        ...
        if (result.status === "PAUSED" && result.pauseReason === "background_transition") {
            await addJob(calcQueue, "resume", { sessionId, reason: "chained_async", type: "calc/session.resume" });
